...
% This command removes the numbering from the title
...
\begin{pkgverbatim}
\begin{verbatim}
% Example: remove exercise sheet numbering
\renewcommand{\uebSerieTitle}{\uebUebungsBlattTitle}
\end{verbatim}
\end{pkgverbatim}
...
\pkgcmddoc{uebSerieTitle}{This macro expands to the title of the exercise sheet. This
  ...
  Or, if you simply want to remove the sheet number,
...
\pkgoptiondoc{nohyperref}{Instructs \texttt{ethuebung} NOT to load packages
  ...
  if you want to disable \texttt{hyperref} completely, use this option. If you get
  problems with the \texttt{xcolor} or \texttt{color} package (e.g. because of option
  clashes), try first to include one of these packages \emph{before} calling
  \cmdname{usepackage\{ethuebung\}}.
}
