...
\begin{pkgnotice}
  ...
  sheet language with \pkgcmd{UebungLanguage} or the \cmdname{[deutsch]} and
  \cmdname{[english]} package options.

  The same remark applies to the~\pkgcmd{hints} and~\pkgcmd{hinweise} commands.
\end{pkgnotice}


\pkgcmddoc[\{$\langle$\textit{symbol}$\rangle$\}]{UebungStarredSymbol}{
...
\pkgcmddoc[\{$\langle$what$\rangle$\}\{$\langle$text$\rangle$\}]{uebHinweisParagraph}{
  ...
  \texttt{$\langle$what$\rangle$} parameter.
